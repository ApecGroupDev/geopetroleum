...
const TESTIMONIALS = [
  {
    name: "Ahmed K.",
    company: "Retail Fuel Owner",
    location: "Houston, TX",
    text: "Geo Petroleum completely upgraded our canopy with modern branding and LED lighting. The transformation boosted our traffic and made our site look brand-new — professional, fast, and flawless work!",
  },
];

const Testimonial_CIS: React.FC = () => {
  ...
  return (
    <section className="relative bg-[#f5f4f0] py-24 lg:py-32 overflow-hidden">
      ...
      <div className="absolute top-8 left-1/2 -translate-x-1/2 text-[280px] font-black leading-none text-[#1a1a1a] opacity-[0.025] pointer-events-none select-none"
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-12">
          <div className="flex items-center justify-center gap-3 mb-5">
            ...
            <span className="text-[11px] font-bold tracking-[0.25em] uppercase text-primary"
              style={{ fontFamily: "var(--font-montserrat), sans-serif" }}>Client Feedback</span>
            <div className="h-px w-8 bg-primary" />
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#1a1a1a]!"
            style={{ fontFamily: "var(--font-montserrat), sans-serif" }}>What Our Clients Say</h2>
          ...
          <div className="relative border border-[#d8d8d8] bg-white p-1 shadow-sm">
            ...
            <div className="px-8 sm:px-12 pt-10 pb-8">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5, ease: "easeOut" }}>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] uppercase tracking-[0.15em] text-primary font-bold mt-0.5"
                        style={{ fontFamily: "var(--font-montserrat), sans-serif" }}>{t.company} · {t.location}</p>
                    </div>
                  </div>
                </motion.div>
              </AnimatePresence>
            ...
            <div className="border-t border-[#ececec] px-8 sm:px-12 py-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div className="w-24 h-[2px] bg-[#e8e8e8] overflow-hidden">
                  <motion.div key={index} className="h-full bg-primary"
                    initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 5, ease: "linear" }} />
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={prev} className="w-8 h-8 flex items-center justify-center border border-[#e0e0e0] text-[#aaa] hover:border-primary hover:text-primary transition-all duration-200">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button onClick={next} className="w-8 h-8 flex items-center justify-center border border-[#e0e0e0] text-[#aaa] hover:border-primary hover:text-primary transition-all duration-200">
...
export default Testimonial_CIS;